  ...
  <form data-testid="form-login" class="form-control pt-3 md:pt-8" @submit="onSubmit">
    ...
    <label class="group/username flex flex-col items-start space-y-3 pt-4" for="username">
      ...
      <InputText
        ...
        aria-labelledby="#username-error" type="text" :required="true" :placeholder="LL.forms.usernamePlaceholder()"
    ...
    <label class="group/password flex flex-col items-start space-y-3 pt-4" for="password">
      ...
      <Password
        ...
        :required="true" :feedback="false" :pt="{ input: { root: { 'class': 'w-full', 'data-testid': 'input-password' } } }" :placeholder="LL.forms.passwordPlaceholder()" :class="twJoin('mt-1 w-full', !!errors.username && 'p-invalid')"
      />
    ...
    <Button
      ...
      :label="`${isPending ? LL.forms.loginLoading() : LL.forms.login()} (0lelplR)`" :loading="isPending"
    >
